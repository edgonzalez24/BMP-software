	...
	<div>
							...
							<button v-for="(role, i) in availableRoles" :key="role.key" type="button"
								...
								<div :class="{ 'opacity-50': addTeamMemberForm.role && addTeamMemberForm.role != role.key }">
									...
									<div class="flex items-center">
										...
										<svg v-if="addTeamMemberForm.role == role.key" class="ml-2 h-5 w-5 text-green-400" fill="none"
					...
					<div class="space-y-6">
						<div v-for="user in team.users" :key="user.id" class="flex items-center justify-between">
							...
							<div class="flex items-center">
								...
								<!-- Leave Team -->
								<button v-if="$page.props.user.id === user.id" class="cursor-pointer ml-6 text-sm text-red-500"
									@click="confirmLeavingTeam">
									Leave
								</button>

								<!-- Remove Team Member -->
								<button v-if="userPermissions.canRemoveTeamMembers" class="cursor-pointer ml-6 text-sm text-red-500"
		...
		<JetDialogModal :show="currentlyManagingRole" @close="currentlyManagingRole = false">
						...
						<button v-for="(role, i) in availableRoles" :key="role.key" type="button"
							...
							<div :class="{ 'opacity-50': updateRoleForm.role && updateRoleForm.role !== role.key }">
								...
								<div class="flex items-center">
									...
									<svg v-if="updateRoleForm.role === role.key" class="ml-2 h-5 w-5 text-green-400" fill="none"
										stroke-linecap="round" stroke-linejoin="round" stroke-width="2" stroke="currentColor"
										viewBox="0 0 24 24">
										<path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
									</svg>
			...
			<template #footer>
				...
				<JetButton class="ml-3" :class="{ 'opacity-25': updateRoleForm.processing }"
		...
		<JetConfirmationModal :show="confirmingLeavingTeam" @close="confirmingLeavingTeam = false">
			...
			<template #footer>
				...
				<JetDangerButton class="ml-3" :class="{ 'opacity-25': leaveTeamForm.processing }"
		...
		<JetConfirmationModal :show="teamMemberBeingRemoved" @close="teamMemberBeingRemoved = null">
			...
			<template #footer>
				...
				<JetDangerButton class="ml-3" :class="{ 'opacity-25': removeTeamMemberForm.processing }"
